 ...
 *
 */
public class RoughIsotopePurityMeasurementDTA {
	...
	public static String description() {
		return "Match DTA file to formula";
	}
	public static String type() {
		return "MZXML";
	}
	public static String parameter_info() {
		return "[DTAFile] [formula] [C12/C13/BOTH] [charge] [ppm] [minPurity] [maxPurity] [minMassCutoff] [maxMassCutoff]";
	}
	...
	public static void execute(String[] args) {
		...
		try {
			...
			String formulaStr = args[1]; // neutral form
			//double monoIsotopicMass = new Double(args[3]);
			String type = args[2];
			int charge = new Integer(args[3]);
			double ppm = new Double(args[4]);
			double minPurity = new Double(args[5]);
			double maxPurity = new Double(args[6]);
			double minMassCutoff = new Double(args[7]);
			double maxMassCutoff = new Double(args[8]);
			...
			IsotopePattern query_peaks = MatchIsotopeToolbox.readDTAFile(inputFile);
			...
			for (String formula: formulaStr.split(",")) {
				...
				if (type.equals("C12")) {
					System.out.println(original_formula + "\t" + charge + "\t" + type + "\tC12Purity:" + c12_simulation_result[0] + "\tMatchScore:" + c12_simulation_result[1]);
				} else if (type.equals("C13")) {
					System.out.println(original_formula + "\t" + charge + "\t" + type + "\tC12Purity:" + c13_simulation_result[0] + "\tMatchScore:" + c13_simulation_result[1]);
				} else if (type.equals("BOTH")){
					System.out.println(original_formula + "\t" + charge + "\t" + type + "\tC12Purity:" + c12_simulation_result[0] + "\tMatchScore:" + c12_simulation_result[1] + "\tC12Purity%:" + c13_simulation_result[0] + "\tMatchScore:" + c13_simulation_result[1]);
				} else {
					System.out.println(original_formula + "\t" + charge + "\t" + type + "\t" + similarity_score + "\t" + c12_simulation_result[0] + "\t" + c12_simulation_result[1] + "\t" + c13_simulation_result[0] + "\t" + c13_simulation_result[1]);
				}
			}
