...
public class RDBERule implements IRule{
    ...
    /**
     * Evaluate whether RBDE works
     * If formula contains elements such as K this might not work
     * @param formula
     * @return
     * @throws CDKException
     */
    public boolean validateRBDE(String formula) throws CDKException {
    	IMolecularFormula molecule = CalculateMonoisotope.getFormula(formula);
    	
    	if (ToolBox.check_formula_valid_element_corrected(formula)) {
    		//System.out.println(formula + "\t" + molecule.toString());
	    	double value = validate(molecule);
	    	if (value > 0) {
	    		return true;
	    	}
    	}
...
//            oxidationStateTable.put("Be", 2);
            ...
            oxidationStateTable.put("Br", new int[]{1});
            oxidationStateTable.put("I", new int[]{1});
            oxidationStateTable.put("K", new int[]{1});
            oxidationStateTable.put("Ca", new int[]{2});
