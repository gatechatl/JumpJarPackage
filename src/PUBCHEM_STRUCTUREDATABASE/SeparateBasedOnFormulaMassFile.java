...
public class SeparateBasedOnFormulaMassFile {

	public static void main(String[] args) {
		try {
			String formula = "C9H11NO2";
			String formula_name = elementOnly(formula);
			System.out.println(formula_name);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	public static void execute(String[] args) {
		...
		try {
				...
				while (in.ready()) {
					...
					//if (ToolBox.check_formula_valid_element(split[1])) {
						...
						*/
						...
						//appendText(str, );
						String key = outputFolder + "/" + mass_category + "/" + formula_name + ".txt";
			...
			while (itr.hasNext()) {
				...
				/*File massDir = new File(filePath);
				if (!massDir.exists()) {
					massDir.mkdir();
				}*/
	        	FileWriter fwriter = new FileWriter(filePath);
	            BufferedWriter out = new BufferedWriter(fwriter);
	            LinkedList list = (LinkedList)map.get(filePath);
